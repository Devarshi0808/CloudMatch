...
def research_products(request, limit=5):
    ...
    if not live["matches"]:
        return {"request":parsed,"status":"abstained","confidence":0.0,"answer":"No listing could be verified from live official-domain search.","evidence":[],"providers":live["providers"],"retrieved_at":live["retrieved_at"],"caveats":[live["disclaimer"]]}
    evidence=[{"id":item["id"],"title":item["title"],"provider":item["provider"],"url":item["url"],"description":item["description"],"relevance_score":item["relevance_score"],"rank_score":item["rank_score"],"verification":item["verification"],"retrieved_at":item["retrieved_at"]} for item in live["matches"]]
    top=evidence[0]
    confidence=round(max(0.0, min(0.99, top["relevance_score"])), 2)
    return {"request":parsed,"status":"grounded","confidence":confidence,"answer":f"Found {len(evidence)} live official-domain result{'s' if len(evidence)!=1 else ''}. First result: {top['title']} on {top['provider'].upper()}.","evidence":evidence,"providers":live["providers"],"retrieved_at":live["retrieved_at"],"caveats":[live["disclaimer"]]}
